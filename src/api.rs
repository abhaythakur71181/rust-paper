...
impl WallhavenClient {
    pub fn new(commands: Command, api_key: Option<String>) -> Result<Self, String> {
        /* Create http client */
        ...
        if let Some(k) = api_key {
            let header_api_value = reqwest::header::HeaderValue::from_str(&k)
                .map_err(|_| String::from("Invalid API key format"))?;
